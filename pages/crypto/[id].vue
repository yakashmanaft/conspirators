...
useHead({
    ...
    const { data: crypto_mesh } = useFetch("/api/funds/crypto_mesh", {
        ...
        transform: (crypto_mesh) => {
            ...
            return crypto_mesh.filter(el => el.id === +route.params.id)[0]
...
<template>
    <Container>
        ...
        <div v-if="crypto_mesh && !accessPlug">
            ...
            <div>
                ...
                <ul>
                    <li v-for="investor in crypto_mesh.invested_mash" @click="link_to_investor(investor.id)">
            ...
            <div>
                <p>{{ crypto_mesh.broker_tag }}</p>
            </div>

            <div>
                <p style="margin: 0;">Тарифы</p>
                <ul>
                    <li>fee: {{ crypto_mesh.fee }}</li>
                    <li>tax: {{ crypto_mesh.tax }}</li>
                    <li>Тариф у брокера: Общий</li>
                </ul>
            </div>
